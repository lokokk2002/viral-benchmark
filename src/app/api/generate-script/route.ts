...
// （callGeminiText 已移除 — 不做 fallback 推斷）
...
// （不做 fallback 推斷 — 拿不到影片就直接失敗，避免浪費 token）
  ...
  try {
    ...
    for (const queueId of ids) {
      try {
        ...
        if (!downloadUrl) {
          const platformName =
            video.platform === "xiaohongshu" ? "小紅書" :
          ...
          await supabase
            ...
            .update({
              status: "failed",
              script_raw: `無法取得影片下載連結（${platformName}）。可能原因：影片已刪除、連結過期、或該平台暫不支援影片下載。`,
            })
            .eq("id", queueId);
          results.push({
            id: queueId,
            status: "failed",
            error: `無法取得${platformName}影片下載連結`,
          });
          continue;
        }
        ...
        if (!videoData) {
          await supabase
            ...
            .update({
              status: "failed",
              script_raw: `影片下載失敗。下載 URL 可能已過期或被平台封鎖。`,
            })
            .eq("id", queueId);
          results.push({
            id: queueId,
        ...
        const raw = await callGeminiWithVideo(
          fileUri,
          videoData.mimeType,
